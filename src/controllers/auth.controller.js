...
/**
 * @desc      Third party signup
 * @param     { Object } req - Request object
 * @param     { Object } res - Response object
 * @property  { Object } req.body - Body data
 * @returns   { JSON } - A JSON object representing the type, message, user data, and tokens
 */
export const thirdpartysignup = catchAsync(async (req, res) => {
  // 1) Calling sign up service
  const { type, message, statusCode, user, tokens } =
    await authService.thirdpartysignup(req.body);
  // 2) Check if something went wrong
  if (type === 'Error') {
    return res.status(statusCode).json({
      type,
...
export const forgotPassword = catchAsync(async (req, res) => {
  ...
  const { type, message, statusCode, resetPasswordToken } =
